...
resource "google_tpu_v2_vm" "tpu_v6_1_bm" {
  ...
  metadata = {
    "startup-script" = <<-EOF
      ...
      echo "GCP_INSTANCE_ID=${var.spanner_instance}" >> /etc/environment
      echo "GCP_DATABASE_ID=${var.spanner_db}" >> /etc/environment
      echo "GCP_REGION=${var.region}" >> /etc/environment
      echo "GCP_INSTANCE_NAME=${google_tpu_v2_vm.tpu_v6_1_bm[count.index].name}" >> /etc/environment
      echo "GCS_BUCKET=${var.gcs_bucket}" >> /etc/environment      
      echo "GCP_QUEUE=vllm-bm-queue-${google_tpu_v2_vm.tpu_v6_1_bm[count.index].accelerator_type}" >> /etc/environment
